...
\input{custom_environments.tex}

\title{Analyzing the Optimality Gap between Linear and Nonlinear Multi-Period OPF Models across Different Sizes of Active Distribution Networks}
...
% To the authors' knowledge, no state-of-the-art research has explored the impact of EDNs' size on the value of the optimality gap. In this regard, this article aims to develop multi-period OPF models for EDNs and simulate its NCP and LP based solution process for different sizes of EDNs viz. 10-bus (small), IEEE-123 bus (medium), and 730-bus (large) test systems. The optimality gaps in the ACOPF solution and decision variables for each network are validated by simulating the control decisions in the OpenDSS platform. The simulation outcome shows that the optimality gap due to LP amplifies with the increasing size of the network but provides near-optimal solutions in each case with fast convergence.  
