...
% \input{../../outputs/custom_environments.tex}
...
% Define \creflist to format references as "(4), (5) and (7)"
% Define \creflist to format references as "(4), (5) and (7)"
\newcommand{\creflist}[1]{%
    \begingroup
    \renewcommand{\creflastconjunction}{ and~}  % Ensures "and" appears only before the last item
    \renewcommand{\crefmiddleconjunction}{, }  % Uses commas between earlier references
    \cref{#1}%
    \endgroup
}
...
In this section, a head to head comparison of the constraints used for both the nonlinear and linear models is provided for the Multi-Period Optimal Power Flow (MPOPF) problem. For the nonlinear formulation, the branch flow model (BFM-NL) \cite{Farivar1} is used, while for the linear formulation, the LinDistFlow variation of the Branch Flow Model \cite{Farivar} is used. It may be seen that barring a few constraints, most of the constraints are common between the two models as described by \crefrange{eq:genCost_withSCD}{eq:modelEndsHere-and-lim_Bj}. The uncommon constraints are suffixed with an `-NL' for the nonlinear model and an `-L' for the linear model.
...
Subject to the constraints \crefrange{eq:RealPowerBalanceNodej-NL}{eq:modelEndsHere-and-lim_Bj} as given below:
...
\addtocounter{equation}{-1} % Decrease the equation counter by 1
...
\vspace{-1.0em} % Adjust the space as needed
...
% \begin{align}
%     {B_{j}^{t}} &= {B_{j}^{t-1} + \Delta t \left( \eta_c P_{c_j}^t - \frac{1}{\eta_d} P_{d_j}^t \right)  }, \quad B_{j}^{0}=B_{j}^{T}  \label{eq:SOC-Trajectory-j}
% \end{align} // currently terminal soc constraint has been retired
...
% Subequations example using alignLsub
\begin{subequations}
...
The network power flow constraints, notably, the nodal real and reactive power balance equations and the KVL constraint across a branch, a re similar across the two models, with the only difference being whether a term representing losses is included (\ref{eq:RealPowerBalanceNodej-NL}, \ref{eq:ReactivePowerBalanceNodej-NL},\ref{eq:KVL-branch-ij-NL}) or not (\ref{eq:RealPowerBalanceNodej-L},\ref{eq:ReactivePowerBalanceNodej-L},\ref{eq:KVL-branch-ij-L}) not.

% The network powerflow constraints, notably, the nodal real and reactive power balance equations and the KVL constraint across a branch are similar across the two models, with the only difference being whether a term representing losses is included \cref{eq:RealPowerBalanceNodej-NL,eq:ReactivePowerBalanceNodej-NL,eq:KVL-branch-ij-NL} or \cref{eq:RealPowerBalanceNodej-L,eq:ReactivePowerBalanceNodej-L,eq:KVL-branch-ij-L} not.  
...
% \cref{eq:lim_PcPdj} also says that the initial and final energy levels for battery must be the same at the end of the optimization time horizon. 
...
% The key advantage of using ENApp in preference to MPOPF is its efficient division of the OPF problem into several smaller problems via efficient exploitation of power flow physics in a radial network \cite{Sadnan}. Typically, Nonlinear Programming problems grow superlinearly in complexity with increase in problem size, and thus division of the OPF problem into smaller, more manageable chunks greatly speeds up the solution process.
...
% The advantage of this smaller computational bottleneck will be readily seen in \Cref{subsec:simulationResults,subsec:scalabilityAnalysis} when compared against MPOPF on the Test System. 
