...
In this section, a head to head comparison of the constraints used for both the nonlinear and linear models is provided for the Multi-Period Optimal Power Flow (MPOPF) problem. For the nonlinear formulation, the branch flow model (BFM-NL) \cite{Farivar1} is used, while for the linear formulation, the LinDistFlow variation of the Branch Flow Model \cite{Farivar} is used. It may be seen that barring a few constraints, most of the constraints are common between the two models as described by \crefrange{eq:genCost_withSCD}{eq:modelEndsHere-and-lim_Bj}.
